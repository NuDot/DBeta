...
// 
// Modifications
// April 28, 2014, Andrey Elagin:
// - moved melec declearation inside the loop to avoid increasingly small masses
//
#include <stdlib.h>
#include <iostream>
...
//input
TString isotope_name="48ca"; //We have 116Cd, 128Te, 130Te and 136Xe. For 160Gd only 0NuBB input data is currently available. 
Int_t mass_number=48;        
Int_t atomic_number=20;       //Cd:48, Te:52, Xe:54, Gd:64
			      //Ca:20, Se:34, Mo42, Nd:60
//end of input 
...
// more isotopes added by AE (Q values are from TIPP2014 talk table):
  else if(A==48 && Z==20) endpoint=4.271; //48Ca
  else if(A==82 && Z==34) endpoint=2.995; //82Se
  else if(A==100 && Z==42) endpoint=3.034; //100Mo
  else if(A==150 && Z==60) endpoint=3.367; //150Nd
...
//  Double_t melec = electron_mass_c2; //MeV already
  ...
  for (Int_t ievent=0; ievent<nevent; ievent++) {    
    ...
    */
    ...
    //cout << e1 << " " << e2 << endl;
    Double_t melec = electron_mass_c2;
    e1 += melec;
    e2 += melec;
    //cout << e1 << " " << e2 << endl;
...
//commented by AE on April 28, 2014
//stucks for Cd
/*  if(!is0NuBB) {
    g2D->GetXaxis()->SetTitle("E1 [MeV]");
    g2D->GetYaxis()->SetTitle("E2 [MeV]");
...
// changed by AE
//  tn_dir->Write();
  tn_dir->Write(0, TObject::kOverwrite);
  fout.Close();
  hepevt_file.close();
